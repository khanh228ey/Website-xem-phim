...
<div class="row container" id="wrapper">
    <div class="halim-panel-filter">
       <div class="panel-heading">
          <div class="row">
             <div class="col-xs-6">
                <div class="yoast_breadcrumb hidden-xs"><span><span><a href="">Danh mục </a> » <span class="breadcrumb_last" aria-current="page">{{$cateSlug->title}}</span></span></span></div>
             </div>
          </div>
       </div>
    ...
    <main id="main-contents" class="col-xs-12 col-sm-12 col-md-8">
       <section>
          <div class="section-bar clearfix">
             <h1 class="section-title"><span>{{$cateSlug->description}}</span></h1>
          </div>
          <div class="section-bar clearfix">
            {{-- <form action="{{route('locphim')}}">
               @csrf
                   ...
                   <a class="halim-thumb" href="{{route('phim',$mv->slug)}}" title="{{$mv->title}}">
                     @php
                        $imageCheck = substr($mv->image,0,5);
                           @endphp
                     @if ($imageCheck == "https")
                     <figure><img class="lazy img-responsive" src="{{$mv->image}}" alt="{{$mv->title}}" title="{{$mv->title}}"></figure>
                    @else
